...
export class Inbal implements OnInit {
    ...
    // Marcas más famosas organizadas por categorías
    ...
    constructor(private http: HttpClient, private metaDatos: Meta) {
        // Effect para debug
        effect(() => {
    ...
    async ngOnInit() {
        this.metaDatos.updateTag({ name: 'description', content: 'Catálogo de comercios del INBAL - Instituto Nacional de Bellas Artes y Literatura. Encuentra tiendas y sucursales en todo México.' });
    ...
    protected async cargarComercios(): Promise<void> {
        try {
            ...
            const response = await this.http.get<Comercio[]>('./inbal/inbal.json').toPromise();
            const data = response || [];
